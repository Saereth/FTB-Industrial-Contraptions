...
import dev.ftb.mods.industrialcontraptions.net.ClearTeleporterPayload;
import dev.ftb.mods.industrialcontraptions.net.ConfigureTeleporterPayload;
import dev.ftb.mods.industrialcontraptions.net.SelectTeleporterPayload;
...
public class TeleporterScreen extends ElectricBlockScreen<TeleporterMenu> {
	...
	protected void init() {
		...
		nameBox = new EditBox(font, leftPos + NAME_X, topPos + NAME_Y, NAME_W, NAME_H,
				Component.translatable("ic.gui.teleporter.name_label"));
		nameBox.setMaxLength(32);
		nameBox.setValue(initialName);
		nameBox.setHint(Component.translatable("block.ic.teleporter.name_hint").withStyle(ChatFormatting.DARK_GRAY));
	...
	protected void extractOverlays(GuiGraphicsExtractor g, int mouseX, int mouseY, float partialTick) {
		...
		drawSmallRedstoneButton(g, leftPos + PUBLIC_BTN_X, topPos + PUBLIC_BTN_Y, mouseX, mouseY,
				be != null && be.isPublic);
		g.text(font, Component.translatable(be != null && be.isPublic ? "ic.gui.teleporter.public_label" : "ic.gui.teleporter.private_label"),
		...
		String statusLabel;
		if (hasLink) {
			String display = be.linkedName != null && !be.linkedName.isEmpty() ? be.linkedName : Component.translatable("ic.gui.teleporter.unnamed").getString();
			statusLabel = truncate(Component.translatable("ic.gui.teleporter.linked_format", formatEntry(display, be.linkedPos, be.linkedDimension)).getString(), statusMax);
		} else {
			statusLabel = truncate(Component.translatable("ic.gui.teleporter.not_linked").getString(), statusMax);
		}
		...
		drawClearButton(g, leftPos + CLEAR_ITEMS_BTN_X, topPos + CLEAR_BTN_Y, CLEAR_ITEMS_BTN_W, Component.translatable("ic.gui.teleporter.clear_storage").getString(), mouseX, mouseY);
		drawClearButton(g, leftPos + CLEAR_FLUIDS_BTN_X, topPos + CLEAR_BTN_Y, CLEAR_FLUIDS_BTN_W, Component.translatable("ic.gui.teleporter.clear_fluids").getString(), mouseX, mouseY);
	...
	private void drawDropdownButton(GuiGraphicsExtractor g, int mouseX, int mouseY) {
		...
		String label = entries().isEmpty()
				? Component.translatable("ic.gui.teleporter.no_teleporters").getString()
				: Component.translatable("ic.gui.teleporter.choose_destination").getString();
	...
	private void drawDropdownOverlay(GuiGraphicsExtractor g, int mouseX, int mouseY) {
		...
		if (entries.isEmpty()) {
			g.text(font, Component.translatable("ic.gui.teleporter.no_teleporters_found").withStyle(ChatFormatting.DARK_GRAY),
	...
	private void extractCustomTooltips(GuiGraphicsExtractor g, int mouseX, int mouseY) {
		...
		if (isIn(mouseX, mouseY, leftPos + PUBLIC_BTN_X, topPos + PUBLIC_BTN_Y, 9, 10)) {
			Component tip = be != null && be.isPublic
					? Component.translatable("ic.gui.teleporter.public_tooltip")
					: Component.translatable("ic.gui.teleporter.private_tooltip");
			g.setTooltipForNextFrame(tip, mouseX, mouseY);
			return;
		}
		boolean hasLink = be != null && be.linkedPos != null && be.linkedDimension != null;
		if (hasLink && isIn(mouseX, mouseY, leftPos + UNLINK_BTN_X, topPos + UNLINK_BTN_Y, 9, 10)) {
			g.setTooltipForNextFrame(Component.translatable("ic.gui.teleporter.unlink_tooltip"), mouseX, mouseY);
			return;
		}
		if (isIn(mouseX, mouseY, leftPos + NAME_X, topPos + NAME_Y, NAME_W, NAME_H)) {
			g.setTooltipForNextFrame(Component.translatable("ic.gui.teleporter.name_tooltip"), mouseX, mouseY);
			return;
		}
		if (dropdownOpen) {
			List<TeleporterEntry> entries = entries();
			...
			for (int i = 0; i < rows; i++) {
				...
				if (isIn(mouseX, mouseY, x + 1, rowY, rowWidth, OVERLAY_ROW_H)) {
					TeleporterEntry e = entries.get(idx);
					String cost = ICUtils.formatEnergy(e.energyUse()).getString();
					g.setTooltipForNextFrame(Component.translatable("ic.gui.teleporter.entry_tooltip",
