...
public class NuclearReactorScreen extends ElectricBlockScreen<NuclearReactorMenu> {
	public static final Identifier NUCLEAR_REACTOR_TEXTURE = IC.id("textures/gui/nuclear_reactor.png");
	...
	protected void extractOverlays(GuiGraphicsExtractor g, int mouseX, int mouseY, float partialTick) {
		...
		g.centeredText(font, (this.menu.isPaused()
						? Component.translatable("ic.reactor.paused")
						: Component.translatable("ic.reactor.energy_output", this.menu.getEnergyOutput()))
						.withStyle(ChatFormatting.WHITE),
				leftPos + 142, topPos + 6, 0xFFFFFF);
		g.centeredText(font, Component.translatable("ic.reactor.heat_percentage", Math.round(this.menu.getHeatFraction() * 100F))
	...
	protected void extractOverlayTooltips(GuiGraphicsExtractor g, int mouseX, int mouseY) {
		...
		if (isIn(mouseX, mouseY, leftPos + 115, topPos + 5, 54, 10)) {
			Component label = this.menu.isPaused()
					? Component.translatable("ic.reactor.tooltip.paused", this.menu.getEnergyOutput())
					: Component.translatable("ic.reactor.tooltip.output", this.menu.getEnergyOutput());
		...
		if (isIn(mouseX, mouseY, leftPos + 115, stripY, 54, 10)) {
			...
			g.setTooltipForNextFrame(Component.translatable("ic.jade.reactor_heat", pct), mouseX, mouseY);
		}
		if (isIn(mouseX, mouseY, leftPos + 105, topPos + 5, 9, 10)) {
			Component label = this.menu.isPaused()
					? Component.translatable("ic.reactor.tooltip.resume")
					: Component.translatable("ic.reactor.tooltip.pause");
			g.setTooltipForNextFrame(label, mouseX, mouseY);
		}
		if (isIn(mouseX, mouseY, leftPos + 105, stripY, 9, 10)) {
			Component label = this.menu.allowRedstone()
					? Component.translatable("ic.reactor.tooltip.redstone_enabled")
					: Component.translatable("ic.reactor.tooltip.redstone_disabled");
			g.setTooltipForNextFrame(label, mouseX, mouseY);
		}
		if (isIn(mouseX, mouseY, leftPos + 94, topPos + 5, 9, 10)) {
			g.setTooltipForNextFrame(Component.translatable("ic.reactor.tooltip.show_jei"), mouseX, mouseY);
		}
	}
	...
	public boolean mouseClicked(MouseButtonEvent event, boolean dragging) {
		...
		if (isIn(mx, my, leftPos + 94, topPos + 5, 9, 10)) {
			ClientRecipeCache.setSearchFilter("@ic reactor");
			return true;
		}
