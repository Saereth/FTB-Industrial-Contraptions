...
import dev.ftb.mods.industrialcontraptions.screen.ElectricBlockMenu;
...
import dev.ftb.mods.industrialcontraptions.screen.BatterySlot;
import dev.ftb.mods.industrialcontraptions.screen.PickaxeSlot;
import dev.ftb.mods.industrialcontraptions.screen.UpgradeSlot;
...
public class ElectricBlockScreen<T extends ElectricBlockMenu> extends AbstractContainerScreen<T> {
	public static final Identifier BASE_TEXTURE = IC.id("textures/gui/base.png");
	...
	protected void tankTooltip(GuiGraphicsExtractor g, int x, int y, int mouseX, int mouseY, FluidStack fluid, int capacity) {
		...
		if (fluid == null || fluid.isEmpty() || capacity <= 0) {
			label = Component.translatable("ic.jade.fluid_empty");
		} else {
	...
	protected void extractDefaultTooltips(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
		if (energyX >= 0 && energyY >= 0 && isIn(mouseX, mouseY, leftPos + energyX, topPos + energyY, 14, 14)) {
			...
			graphics.setTooltipForNextFrame(
					Component.literal(ICUtils.formatEnergy(cur).getString()
							+ " / " + ICUtils.formatEnergy(cap).getString()),
					mouseX, mouseY);
		}
		if (drawDefaultArrow && isIn(mouseX, mouseY, leftPos + 80, topPos + 35, 22, 16)) {
			int pct = Math.round(this.menu.getProgressFraction() * 100F);
			graphics.setTooltipForNextFrame(Component.translatable("ic.jade.progress", pct), mouseX, mouseY);
		}
	}
	...
	public void drawTank(GuiGraphicsExtractor g, int x, int y, FluidStack fluid, int capacity) {
		...
		// removed the IClientFluidTypeExtensions#getStillTexture API; since IC tanks only
