...
import dev.ftb.mods.industrialcontraptions.screen.PumpMenu;
...
public class PumpBlockEntity extends DiggingBaseBlockEntity {
	...
	public PumpBlockEntity(BlockPos pos, BlockState state) {
		super(ICElectricBlocks.PUMP, pos, state);
	}
	...
	public int getTankCapacity() {
		return ICConfig.MACHINES.PUMP_TANK_CAPACITY.get();
	}
	...
	public void digBlock(BlockState state, BlockPos miningPos) {
		...
		storedFluid = f;
		fluidAmount += 1000;
		level.setBlock(miningPos, ICBlocks.EXFLUID.get().defaultBlockState(), 3);
		setChanged();
