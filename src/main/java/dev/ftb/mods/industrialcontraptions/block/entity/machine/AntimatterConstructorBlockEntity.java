...
public class AntimatterConstructorBlockEntity extends ElectricBlockEntityRef {
	...
	public AntimatterConstructorBlockEntity(BlockPos pos, BlockState state) {
		super(ICElectricBlocks.ANTIMATTER_CONSTRUCTOR, pos, state);
	}
	...
	private double boostFor(ItemStack stack) {
		...
		RecipeType<AntimatterBoostRecipe> type =
				(RecipeType<AntimatterBoostRecipe>) (RecipeType<?>) ICRecipes.ANTIMATTER_BOOST.get();
	...
	public void tick() {
		...
		double boostMult = ICConfig.MACHINES.ANTIMATTER_CONSTRUCTOR_BOOST.get();
		...
		if (progress >= PRODUCTION_THRESHOLD) {
			ItemStack result = new ItemStack(ICItems.ANTIMATTER.item.get());
