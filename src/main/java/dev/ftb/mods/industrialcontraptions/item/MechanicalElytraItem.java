...
public class MechanicalElytraItem extends Item implements EnergyItemHandler {
	...
	public double getEnergyCapacity(ItemStack stack) {
		return ICConfig.EQUIPMENT.MECHANICAL_ELYTRA_CAPACITY.get();
	}
	...
	public void inventoryTick(ItemStack stack, ServerLevel level, Entity entity, EquipmentSlot slot) {
		...
		double rechargeRate = ICConfig.EQUIPMENT.MECHANICAL_ELYTRA_RECHARGE.get();
	...
	private void tickFlight(ItemStack stack, LivingEntity le) {
		double drain = ICConfig.EQUIPMENT.ARMOR_FLIGHT_ENERGY.get();
		...
		if (flightTicks >= 3 && le.isCrouching()) {
			...
			damageEnergyItem(stack, ICConfig.EQUIPMENT.ARMOR_FLIGHT_STOP.get());
			...
			damageEnergyItem(stack, ICConfig.EQUIPMENT.ARMOR_FLIGHT_BOOST.get());
		}
	}
