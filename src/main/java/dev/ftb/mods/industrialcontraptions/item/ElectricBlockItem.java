...
public class ElectricBlockItem extends BlockItem {
	...
	public void appendHoverText(ItemStack stack, TooltipContext context, TooltipDisplay display,
		...
		if (maxIn > 0D) {
			tooltip.accept(Component.translatable("ic.max_input", EnergyItemHandler.formatEnergy(maxIn))
					.withStyle(ChatFormatting.GRAY));
		}

		double maxOut = instance.maxEnergyOutput.get();
		if (maxOut > 0D) {
			tooltip.accept(Component.translatable("ic.energy_output", EnergyItemHandler.formatEnergy(maxOut))
					.withStyle(ChatFormatting.GRAY));
		}

		double cap = instance.energyCapacity.get();
		if (cap > 0D) {
			tooltip.accept(Component.translatable("ic.energy_capacity", EnergyItemHandler.formatEnergy(cap))
					.withStyle(ChatFormatting.GRAY));
		}

		double use = instance.energyUsage.get();
		if (use > 0D) {
			tooltip.accept(Component.translatable("ic.energy_usage", EnergyItemHandler.formatEnergy(use))
