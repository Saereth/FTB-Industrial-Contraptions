package dev.ftb.mods.industrialcontraptions.entity;
...
public final class NukeArrowEntity extends AbstractArrow {
	...
	public NukeArrowEntity(Level level, LivingEntity shooter) {
		super(ICEntities.NUKE_ARROW.get(), level);
	...
	protected ItemStack getDefaultPickupItem() {
		return new ItemStack(ICItems.NUKE_ARROW.get());
	}
	...
	private void detonate(Vec3 at) {
		if (level() instanceof ServerLevel server) {
			double radius = ICConfig.NUCLEAR.NUKE_RADIUS.get();
			BlockPos pos = BlockPos.containing(at);
			if (ICConfig.NUCLEAR.NUKE_RESPECTS_CLAIMS.get()) {
