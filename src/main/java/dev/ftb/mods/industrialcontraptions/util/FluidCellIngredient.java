...
public record FluidCellIngredient(Fluid fluid) implements ICustomIngredient {
	...
	public boolean test(ItemStack stack) {
		if (!stack.is(ICItems.FLUID_CELL.get())) return false;
	...
	public Stream<Holder<Item>> items() {
		return Stream.of(ICItems.FLUID_CELL.get().builtInRegistryHolder());
	}

	@Override
	public SlotDisplay display() {
		ItemStack filled = new ItemStack(ICItems.FLUID_CELL.get());
	...
	public IngredientType<?> getType() {
		return ICIngredientTypes.FLUID_CELL.get();
	}
}
