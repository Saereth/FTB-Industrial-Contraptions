...
public final class NuclearExplosion {
	...
	public static void detonate(ServerLevel level, BlockPos center, double radius, UUID ownerId, String ownerName) {
			...
			for (int dz = -rxz; dz <= rxz; dz++) {
				for (int y = yHi; y >= yLo; y--) {
					...
					try {
						...
						IC.LOGGER.warn("Error reading block state during nuclear explosion at {}", mpos, ex);
					...
					if (state.isAir()) {
						kind = FLAG_AIR;
					} else if (state.is(ICUtils.REINFORCED) || state.getDestroySpeed(level, mpos) < 0F) {
		...
		IC.LOGGER.warn("Nuclear explosion by {}/{} at {} radius {} destroyed {} blocks (volume {}) in {}ms",
