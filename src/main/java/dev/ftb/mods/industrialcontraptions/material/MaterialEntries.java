package dev.ftb.mods.industrialcontraptions.material;
		...
		for (Material material : Material.values()) {
			...
			for (MaterialComponent component : material.components()) {
				...
				if (component.isBlock()) {
					DeferredBlock<Block> block = ICBlocks.REGISTRY.register(name,
							id -> new Block(blockProps(component, material.tool())
									.setId(ResourceKey.create(Registries.BLOCK, id))));
					DeferredItem<Item> item = ICItems.REGISTRY.register(name,
