...
public class CentrifugeTankHandler extends SnapshotJournal<CentrifugeTankHandler.Contents> implements ResourceHandler<FluidResource> {
	private final CentrifugeBlockEntity machine;
	public record Contents(FluidStack input, FluidStack output) {}

	public CentrifugeTankHandler(CentrifugeBlockEntity machine) { this.machine = machine; }
	...
	@Override public long getCapacityAsLong(int index, FluidResource resource) { return index >= 0 && index < 2 ? CentrifugeBlockEntity.TANK_CAPACITY : 0; }
	...
	public int insert(int index, FluidResource resource, int amount, TransactionContext transaction) {
		...
		int inserted = Math.min(amount, CentrifugeBlockEntity.TANK_CAPACITY - input.getAmount());
