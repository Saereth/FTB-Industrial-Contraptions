...
By default the pickaxe never takes durability damage. To mirror vanilla behaviour where each block mined costs one durability, set `quarry_pickaxe_takes_damage = true` in `ic-common.toml`. If the pickaxe breaks, the slot empties and mining falls back to the no-tool drop table.
