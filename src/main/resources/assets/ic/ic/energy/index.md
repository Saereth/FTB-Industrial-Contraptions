...
<Column alignItems="center" fullWidth={true}>
  ...
  Industrial Contraptions runs on its own power unit — the **zap**. Every cable, battery, and machine uses zaps internally. Industrial Contraptions generators and cables can power FE-consuming machines directly; the opposite direction — feeding FE **into** an Industrial Contraptions machine — requires a <ItemLink id="lv_rectifier" />.

  When Industrial Contraptions pushes into an FE consumer through a cable, the flow is capped at the cable tier's transfer rate. An LV cable running into an FE machine that would happily drink thousands of FE per tick still only delivers 32 zaps per tick worth of FE, keeping the cable safe instead of burning it out.
</Column>
